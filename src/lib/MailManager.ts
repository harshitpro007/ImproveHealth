...
export class MailManager {
	...
	async eligibilityChecksMail(params) {

		const templatePath = SERVER.TEMPLATE_PATH;
		const template = templatePath + 'eligibilityCheck.html';

		const mailContent = await new TemplateUtil(template).compileFile({
			name: params?.name,
			disputeNumber: params?.disputeNumber,
			link: SERVER.ADMIN_URL
		});
		const emailParams = {
			email: params.email,
			subject: TEMPLATES.EMAIL.SUBJECT.UPDATE_ELIGIBILTY + ": " + params?.disputeNumber,
			content: mailContent,
		};
		return await this.sendMail(emailParams);
	...
	async eligibilityChecksErrorMail(params) {

		const templatePath = SERVER.TEMPLATE_PATH;
		const template = templatePath + 'eligibilityCheckError.html';
	...
	/**
	 * @function unassignedCassesFileMail
	 * @description admin export unassigned cases listing
	 * @author Chitvan Baish
	 * @param params.name: sub-admin's name
	 * @param params.email: sub-admin's email
	 * @param params.link: pdf link
	 */
	async exportCassesFileMail(params) {
		let mailContent = await (new TemplateUtil(SERVER.TEMPLATE_PATH + "unassigned-cases.html"))
			.compileFile({
				"name": params.name,
				"email": params.email,
				"link": params.link,
		...
		const emailParams = {
			email: params.email,
			subject: TEMPLATES.EMAIL.SUBJECT.EXPORT_FILES,
			content: mailContent,
		};
		return await this.sendMail(emailParams);
	}
