  ...
  public async createTopic(): Promise<void> {
    try {
      ...
      await this.admin.createTopics({
        topics: [{ topic: KAFKA_TOPICS_PRODUCER.NOTIFICATION_REQUEST }],
      });
      console.log(` ****** Topic created successfully.  ${KAFKA_TOPICS_PRODUCER.NOTIFICATION_REQUEST} ****** `);
    } catch (error) {
      console.error(`Error creating topic in ${KAFKA_TOPICS_PRODUCER.NOTIFICATION_REQUEST}:`, error);
    } finally {
...
// Example usage:
export const kafkaProducerNotification = new KafkaProducer(
